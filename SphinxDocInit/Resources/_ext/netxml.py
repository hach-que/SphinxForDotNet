...
def convert_to_keyword_if_possible(name):
  if name == "String":
    return "string"
  if name == "Single":
    return "float"
...
""")
  ...
  for inherit in elem.findall("Inherits"):
    content.append(i1 + ":inherits: " + json.dumps((
      convert_to_keyword_if_possible(inherit.get("TypeName")),
      inherit.get("TypeFullName"))))
    
  implements = []
  for implement in elem.findall("Implements"):
    implements.append((
      convert_to_keyword_if_possible(implement.get("TypeName")),
      implement.get("TypeFullName")))
  ...
  for field in elem.findall("Field"):
    ...
    content.append(i2 + ":type: " + json.dumps((
      convert_to_keyword_if_possible(field.get("TypeName")),
      field.get("TypeFullName"))))
    content.append(i2)
  ...
  for prop in elem.findall("Property"):
    ...
    content.append(i2 + ":type: " + json.dumps((
      convert_to_keyword_if_possible(prop.get("TypeName")),
      prop.get("TypeFullName"))))
    content.append(i2)
  ...
  for method in elem.findall("Method"):
    ...
    for parameter in parameters:
      anchorparams.append(
        convert_to_keyword_if_possible(parameter.get("TypeName")))
    ...
    for parameter in parameters:
      parameters_for_json.append((
        convert_to_keyword_if_possible(parameter.get("TypeName")),
        parameter.get("Name"),
        parameter.get("TypeFullName"),
        parameter.get("IsRef"),
        parameter.get("IsOut")))
    
    content.append(i2 + ":parameters: " + json.dumps(parameters_for_json))
    content.append(i2 + ":return: " + json.dumps((
      convert_to_keyword_if_possible(method.get("ReturnTypeName")),
      method.get("ReturnTypeFullName"))))
...
def load_xml(app):
  global dotnet_docs
  matching = glob.glob("*.combined.xml")
  if len(matching) == 0:
    app.warn(
      "THERE IS NO .combined.xml!  You need to add one file " + 
      "with the .combined.xml extension to the root of this " 
      "documentation project in order for .NET API documentation " 
      "to be generated.")
    return
  with open(matching[0], 'r') as content_file:
