...
namespace SphinxDataGenerator
{
    class MainClass
    {
        ...
        {
            var targetAssembly = new FileInfo(args[0]);
            var dependencies = args[1].Split(';').Select(x => new FileInfo(x)).ToArray();

            var toProcess = new List<DocumentationToProcess>();
            foreach (var asm in new[] { targetAssembly }.Concat(dependencies))
            {
                if (asm.Name.Contains("SphinxDataGenerator"))
                {
                    continue;
                }

                var docFile = asm.FullName.Replace(
            ...
            var outputPath = toProcess[0].DocumentationFile.Replace(".xml", ".combined.xml");
            Console.WriteLine("## Saving combined documentation to " + outputPath);
            ...
            foreach (var entry in toProcess)
            {
                ...
                if (assembly == null)
                {
                    assembly = Assembly.LoadFrom(entry.AssemblyFile);
                }

                entry.SupportsModules = assembly.GetCustomAttribute<SupportsDocumentationModulesAttribute>() != null;

                var documentationPath = entry.DocumentationFile;
